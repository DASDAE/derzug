...
class TestAnnotationsWidget:
    ...
    def test_colliding_ids_replace_existing_annotations(self, widget, monkeypatch):
        ...
        assert (
            widget._entries[0].annotation_set.annotations[0].geometry.values == (9.0,)
        )
    ...
    def test_waterfall_resends_update_selected_annotations_row(self, qtbot):
        """A second Waterfall send should refresh the same selected row in Annotations."""
        with widget_context(Waterfall) as waterfall, widget_context(Annotations) as store:
            waterfall.show()
            store.show()
            qtbot.wait(10)
    ...
    def test_waterfall_local_annotations_do_not_import_until_s(self, qtbot):
        """Connected Waterfall annotations should stay local until the user presses s."""
        with widget_context(Waterfall) as waterfall, widget_context(Annotations) as store:
            waterfall.show()
            store.show()
            qtbot.wait(10)
            ...
            qtbot.waitUntil(lambda: QApplication.focusWidget() is waterfall, timeout=1000)
